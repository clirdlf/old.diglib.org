...
# find . \( -name "*.htm" -or -name "*.html" \) -exec ./replace-100701.pl {} \; | tee replace.100701.txt

print "."; # to let us know we are alive and checking

$old = $ARGV[0]; # assuming one filename in the first argument

$date = "100701"; # not a real date, but more of an identifier for this script
...
while (<OLD>) {
	# our tests
	if (/function MM_preloadImages/) {
		$safetochange = 1;
	}
    if (/\<\/head\>/) {
    	if ($safetochange == 1) {
	    	s/\<\/head\>/\<link rel="stylesheet" type="text\/css" href="\/wrap\/wrap.css" \/\>\<script type="text\/javascript" src="\/wrap\/wrap.js"\>\<\/script\>\<\/head\>/g;
	    	&report($.,"head",$_);
    	}
    }
    if (/\"MM_preloadImages/) {
    	s/\"MM_preloadImages/\"wrapoldcontent();MM_preloadImages/g;
    	&report($.,"onload",$_);
    }
