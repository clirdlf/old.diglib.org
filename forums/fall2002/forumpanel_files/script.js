...
var soundFile = ""; 
var prevSoundFile = "";
var soundFlag = 0; //1 for play, 0 for pause
function ToggleSound( option, object ) {
	if ( !object || !SoundEnabled )
		return;
	if ( prevSoundFile == object ) { 
				if ( object.IsPlaying() ) {
					object.pause();
					soundFlag = 0;
				}
...
var g_baseURL = gtmpstr.substr(0, gtmpstr.lastIndexOf("/") ) + "/" + "forumpanel_files";
var g_showoutline = 1;
  ...
  if ( (msie || isnav ) && ( (os < 0 && ver >= 4 ) || ( os >= 0 && ver >= 5 ) || (os >=0 && msie < 0 && ver >= 4 ) )  ){
	if ( msie >= 0 )  {
		if ( isIEFiles > 0 )
			...
			window.location.replace(  "forumpanel_files/error.htm" );
	}
	else {
